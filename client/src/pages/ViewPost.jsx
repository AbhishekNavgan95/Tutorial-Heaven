...
const ViewPost = () => {
    const { GET_POST, GET_POST_COMMENTS } = postEndpoints;
    const { GET_ALL_CATEGORY_POSTS } = dataEndpoints;
    ...
    const getPosts = useCallback(async () => {
        if (postId) {
            try {
                const response = await apiConnector("GET", `${GET_POST}/${postId}`);
                setPost(response?.data?.data);
                if (response?.data?.data?.category?.image?.url) {
                    const bannerUrl = getCloudinaryUrl(response.data.data.category.image.url, 600, 200);
                    setCategoryBanner(bannerUrl);
                }
            } catch (e) {
                console.log("error : ", e);
    ...
    const getCategoryPosts = useCallback(async () => {
        if (post.category) {
            try {
                ...
                console.log("error : ", e);
                toast.error("Couldn't get related posts", {
                    style: {
                        border: "1px solid #5252B7",
                        padding: "8px 16px",
                        color: "#DFE2E2",
        ...
        <div className='pt-[4rem] min-h-[calc(100vh-6rem)] px-3 relative mx-auto w-full text-night-900'>
            <div className='grid grid-cols-1 lg:grid-cols-[1fr,auto] gap-x-5 w-full'>
                ...
                <section className='h-max flex flex-col items-start gap-3 py-3 row-span-2 w-full lg:min-w-[500px] lg:max-w-[500px] '>
                    ...
                    <img src={categoryBanner} loading='lazy' className='w-full h-[100px] bg-night-50 object-cover rounded-lg ' alt="thumbnail" />
                    ...
                    {similarPostsCurrentPage < similarPostsTotalPages && (
                        <button onClick={() => similarPostsCurrentPage < similarPostsTotalPages ? setSimilarPostsCurrentPage(similarPostsCurrentPage + 1) : null}>
                            Load more
                        </button>
                    )}
