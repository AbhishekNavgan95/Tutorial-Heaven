...
const Post = ({ post }) => {
  ...
  return (
    <div onClick={() => navigate(`/view/${post?._id}`)} className='flex flex-col xl:flex-row gap-3 cursor-pointer text-night-900 transition-all duration-300 rounded-lg group'>
      <span className='w-full h-[180px] xl:h-auto flex items-center justify-center xl:max-w-[35%] xl:aspect-video overflow-hidden rounded-lg'>
        <img className='w-full group-hover:scale-105 transition-all duration-300 object-cover rounded-lg' src={post?.thumbnail?.url} alt="" />
      </span>
